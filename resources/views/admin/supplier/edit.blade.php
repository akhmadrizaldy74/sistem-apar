<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-slate-900 tracking-tight">Edit Supplier</h2>
                <p class="text-sm text-slate-500 font-semibold">Perbarui informasi supplier {{ $supplier->nama_supplier }}.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            <form action="{{ route('admin.suppliers.update', $supplier) }}" method="POST" class="p-8 sm:p-12 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="kontak_person" id="kontak_person" value="{{ old('kontak_person', $supplier->kontak_person) }}"
                            class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 font-bold text-slate-900 placeholder:text-slate-300 transition outline-none"
                    ...
                    <div>
                        ...
                        <input type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon', $supplier->no_telepon) }}"
                            class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 font-bold text-slate-900 placeholder:text-slate-300 transition outline-none"
                    ...
                    <div>
                        ...
                        <input type="text" name="no_wa" id="no_wa" value="{{ old('no_wa', $supplier->no_wa) }}" required
                            class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 font-bold text-slate-900 placeholder:text-slate-300 transition outline-none"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" id="alamat" rows="3"
                            ...
                            placeholder="Masukkan alamat gudang / kantor supplier">{{ old('alamat', $supplier->alamat) }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex items-center gap-6">
                            <label class="inline-flex items-center gap-3 cursor-pointer">
                                <input type="radio" name="status" value="aktif" {{ old('status', $supplier->status) === 'aktif' ? 'checked' : '' }}
                            ...
                            <label class="inline-flex items-center gap-3 cursor-pointer">
                                <input type="radio" name="status" value="nonaktif" {{ old('status', $supplier->status) === 'nonaktif' ? 'checked' : '' }}
                                    class="w-5 h-5 text-red-600 border-slate-300 focus:ring-red-500">
                                <span class="text-sm font-bold text-slate-900">
