<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight">Tambah Produk</h2>
                <p class="text-base font-semibold text-gray-700">Tambahkan produk APAR baru ke katalog admin.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.produk.store') }}" method="POST" enctype="multipart/form-data" class="p-12">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="jenis_apar_id" id="jenis_apar_id" required
                                ...
                                @foreach($jenisApars as $j)
                                    <option value="{{ $j->id }}" {{ old('jenis_apar_id') == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="harga" id="harga" value="{{ $formatRupiahInput(old('harga')) }}" required inputmode="numeric"
                        ...
                        <div>
                            ...
                            <div class="relative group">
                                ...
                                <label for="gambar" class="cursor-pointer flex flex-col items-center justify-center w-full h-44 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 group-hover:border-red-600/30 transition-all overflow-hidden relative">
                                    <div class="flex flex-col items-center text-gray-400 group-hover:text-red-600 transition" id="upload-placeholder">
                                        ...
                                        <span class="text-xs font-black uppercase tracking-widest">Upload Photo</span>
                                    </div>
