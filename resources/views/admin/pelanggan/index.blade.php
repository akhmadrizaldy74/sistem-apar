            ...
            <div class="bg-white/80 backdrop-blur-md rounded-3xl border border-white/60 p-8 shadow-xl shadow-slate-200/50 md:col-span-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-slate-700 mt-1 leading-relaxed">Data pelanggan kini bisa menampilkan email pemulihan dan nomor WhatsApp agar login pelanggan lebih mudah dipantau.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white/60 overflow-hidden">
            ...
            <form method="GET" class="p-6 flex flex-col sm:flex-row gap-3 bg-slate-50/60 backdrop-blur-sm border-b border-gray-100/70">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, email, WhatsApp, atau perusahaan..."
                    ...
                    <tbody class="divide-y divide-gray-100/70">
                        @foreach($pelanggans as $p)
                            <tr class="hover:bg-red-50/30 transition-colors group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] font-medium text-slate-400 mt-0.5">{{ $p->created_at ? $p->created_at->format('d M Y') : '-' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    @if($p->user?->email)
                                        <p class="text-xs font-bold text-slate-600">{{ $p->user->email }}</p>
                                    @else
                                ...
                                <td class="px-8 py-6">
                                    <span class="inline-flex px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border bg-emerald-50 text-emerald-700 border-emerald-100">
                                        Pelanggan
                                    </span>
                                </td>
                                <td class="px-8 py-6">
                                    <div>
                                        <p class="text-xs font-bold text-slate-600">{{ ucfirst($p->status ?? 'calon') }}</p>
                                        <p class="text-[10px] font-medium text-slate-400 mt-0.5 truncate max-w-xs">{{ $p->alamat ?: '-' }}</p>
                                    </div>
                                </td>
                    ...
                    <article class="p-5">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-xs {{ $p->user?->email ? 'text-slate-500' : 'text-slate-400' }}">{{ $p->user?->email ?: 'Belum ada email' }}</p>
                                <p class="mt-0.5 text-xs text-slate-400">Status: {{ ucfirst($p->status ?? 'calon') }} • Terdaftar: {{ $p->created_at ? $p->created_at->format('d M Y') : '-' }}</p>
                            </div>
                        </div>
