<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-slate-900 tracking-tight">Buat Purchase Order</h2>
                <p class="text-sm text-slate-500 font-semibold">Buat pengajuan Purchase Order (PO) baru untuk dikirim ke supplier.</p>
            </div>
        </div>
    </x-slot>

    <form action="{{ route('admin.purchase-orders.store') }}" method="POST" id="po-form" class="space-y-8">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6 h-fit">
                <div class="border-b border-slate-100 pb-4">
                    ...
                    <p class="text-xs text-slate-400 font-semibold">Data supplier & tanggal dokumen</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="catatan" id="catatan" rows="4"
                        ...
                        placeholder="Catatan tambahan untuk supplier...">{{ old('catatan') }}</textarea>
            ...
            <div class="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
                <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 font-semibold">Pilih kategori dan nama item dari database</p>
                    </div>
                ...
                <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100">
                    <a href="{{ route('admin.purchase-orders.index') }}" class="px-8 py-4 bg-slate-600 text-white font-black rounded-2xl hover:bg-slate-700 transition uppercase tracking-widest text-xs">
                        Batal
                    </a>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            async function populateItemDropdown(row, kategori, selectedNama = '') {
                ...
                items.forEach(item => {
                    ...
                    if (selectedNama && selectedNama === item.nama) {
                        opt.selected = true;
                    }
                    selectNama.appendChild(opt);
                });

                if (selectNama.selectedOptions.length > 0 && selectNama.selectedOptions[0].value !== '') {
                    const selectedOpt = selectNama.selectedOptions[0];
                    if (!hiddenHarga.value || parseFloat(hiddenHarga.value) === 0) {
                        setHargaValue(row, selectedOpt.dataset.harga || 0);
                    }
                } else if (items.length > 0 && !selectedNama) {
                    selectNama.selectedIndex = 1;
                    const selectedOpt = selectNama.selectedOptions[0];
                    setHargaValue(row, selectedOpt.dataset.harga || 0);
                }
            ...
            const initialKategori = @js($selectedKategori ?? 'produk');
            const initialItemNama = @js($selectedItemNama ?? '');

            // Populate initial row
            addRow(initialItemNama, initialKategori);
        });
    </script>
    @endpush
</x-app-layout>
