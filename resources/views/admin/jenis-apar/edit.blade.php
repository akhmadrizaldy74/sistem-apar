<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight">Edit Jenis APAR</h2>
                <p class="text-sm text-gray-500 font-medium">Perbarui kategori APAR: {{ $jenisApar->nama }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.jenis-apar.update', $jenisApar) }}" method="POST" class="p-12 space-y-8">
                @csrf
                @method('PUT')
                <div>
                    <label for="nama" class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Nama Jenis APAR</label>
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $jenisApar->nama) }}" required
