<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight">Edit Unit APAR</h2>
                <p class="text-sm text-gray-500 font-medium">Perbarui informasi teknis unit: {{ $unitApar->no_seri }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.unit-apar.update', $unitApar) }}" method="POST" class="p-12">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="pelanggan_id" id="pelanggan_id" required
                                ...
                                @foreach($pelanggans as $p)
                                    <option value="{{ $p->id }}" {{ old('pelanggan_id', $unitApar->pelanggan_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="produk_id" id="produk_id" required
                                ...
                                @foreach($produks as $pr)
                                    <option value="{{ $pr->id }}" {{ old('produk_id', $unitApar->produk_id) == $pr->id ? 'selected' : '' }}>{{ $pr->nama }} • {{ $pr->kapasitas ?? '-' }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="no_seri" id="no_seri" value="{{ old('no_seri', $unitApar->no_seri) }}" required
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="tgl_beli" id="tgl_beli" value="{{ old('tgl_beli', optional($unitApar->tgl_beli)->format('Y-m-d') ?: date('Y-m-d')) }}" required
                                class="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-red-600/20 font-bold text-gray-900 transition">
