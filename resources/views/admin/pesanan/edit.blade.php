<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.pesanan.show', $pesanan->id) }}" class="p-2.5 bg-white rounded-2xl border border-slate-200 text-slate-400 hover:text-red-700 hover:border-red-200 transition shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            </a>
            <div>
                <h2 class="text-3xl font-black text-slate-900 tracking-tight">Edit Pesanan</h2>
                <p class="text-sm text-slate-500 font-semibold">Perbarui tipe pesanan dan rincian pesanan {{ $pesanan->no_pesanan ?: ('# ' . $pesanan->id) }}.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl" x-data="{
        tipe: '{{ old('tipe_pesanan', $pesanan->tipe_pesanan_effective) }}',
        pelangganId: '{{ old('pelanggan_id', $pesanan->pelanggan_id) }}'
    }">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            <form action="{{ route('admin.pesanan.update', $pesanan->id) }}" method="POST" class="p-8 sm:p-12 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="bg-slate-50/80 p-6 rounded-3xl border border-slate-200/60 space-y-4">
                    <label class="text-xs font-black text-slate-500 uppercase tracking-widest block">
                        Tipe Pesanan <span class="text-red-600">*</span>
                    </label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="pelanggan_id" id="pelanggan_id" x-model="pelangganId" required
                            ...
                            @foreach($pelanggans as $pelanggan)
                                <option value="{{ $pelanggan->id }}" {{ old('pelanggan_id', $pesanan->pelanggan_id) == $pelanggan->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', optional($pesanan->tanggal)->format('Y-m-d')) }}" required
                            class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 font-bold text-slate-900 transition outline-none">
                ...
                @php
                    ...
                    $existingProdukId = $firstDetail?->produk_id;
                    $existingJumlah = $firstDetail?->jumlah ?? 1;
                @endphp
                ...
                <div x-show="tipe === 'jual_produk'" x-transition class="space-y-6 pt-4 border-t border-slate-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div class="sm:col-span-2">
                            ...
                            <select name="produk_id" id="produk_id"
                                ...
                                @foreach($produks as $produk)
                                    <option value="{{ $produk->id }}" {{ old('produk_id', $existingProdukId) == $produk->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="jumlah" id="jumlah" min="1" value="{{ old('jumlah', $existingJumlah) }}"
                                class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-red-500 focus:ring-1 focus:ring-red-500 font-bold text-slate-900 transition outline-none">
                        ...
                        <select name="unit_apar_id" id="unit_apar_id"
                            ...
                            @foreach($unitApars as $unit)
                                <option value="{{ $unit->id }}"
                                    ...
                                    {{ old('unit_apar_id', $pesanan->unit_apar_id) == $unit->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="jasa_id" id="jasa_id"
                            ...
                            @foreach($jasas as $jasa)
                                <option value="{{ $jasa->id }}" {{ old('jasa_id', $pesanan->jasa_id) == $jasa->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="catatan_admin" id="catatan_admin" rows="3"
                        ...
                        placeholder="Catatan tambahan pesanan admin...">{{ old('catatan_admin', $pesanan->catatan_admin) }}</textarea>
                    <x-input-error :messages="$errors->get('catatan_admin')" class="mt-2" />
                </div>

                <!-- Buttons -->
                <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-100">
                    <a href="{{ route('admin.pesanan.show', $pesanan->id) }}" class="px-8 py-4 bg-slate-600 text-white font-black rounded-2xl hover:bg-slate-700 transition uppercase tracking-widest text-xs">
                        Batal
                    </a>
