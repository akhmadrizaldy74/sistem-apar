...
<div class="space-y-8">
    <div class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 class="text-lg font-black text-slate-900">Informasi Jasa / Service</h3>
            <div class="mt-6 space-y-5">
                <div>
                    <label for="nama" class="block text-[10px] font-black uppercase tracking-widest text-slate-400">Nama Jasa / Service</label>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 class="text-lg font-black text-slate-900">Peralatan Service (Opsional)</h3>
            <p class="mt-2 text-sm font-semibold text-slate-500">Pilih peralatan yang dipakai (jika ada). Kosongkan jika layanan ini hanya berupa tarif jasa tanpa sparepart.</p>
            <div class="mt-6 space-y-4">
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 px-4 py-6 text-sm font-semibold text-slate-500">
                        Belum ada peralatan service. Tambahkan dulu lewat menu Peralatan.
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('admin.jasa.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-xs font-black uppercase tracking-widest text-slate-500 transition hover:bg-slate-100 hover:text-slate-700">Batal</a>
        <button type="submit" class="rounded-2xl bg-red-600 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-red-600/20 transition hover:bg-red-700">
            {{ $isEdit ? 'Simpan Perubahan' : 'Simpan Jasa / Service' }}
        </button>
    </div>
</div>
