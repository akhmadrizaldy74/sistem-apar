<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight">Edit Data Service APAR</h2>
                <p class="text-sm text-gray-500 font-medium">Ubah data service APAR, lalu klik Simpan.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-5xl" x-data="{
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.service.update', $service) }}" method="POST" class="p-12">
                ...
                @if($service->status_konfirmasi === 'pending' && !$service->service_paket_id)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        <div>
                            ...
                            <select name="unit_apar_id" id="unit_apar_id" required
                                ...
                                @foreach($units as $unit)
                                    <option value="{{ $unit->id }}" @selected(old('unit_apar_id', $service->unit_apar_id) == $unit->id)>
                                        {{ $unit->no_seri }} - {{ $unit->pelanggan?->nama ?? '-' }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="tgl_service" id="tgl_service" value="{{ old('tgl_service', optional($service->tgl_service)->format('Y-m-d')) }}" required
                                class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-red-500/30 focus:border-red-400 font-bold text-gray-900 transition">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="jenis_service" id="jenis_service" value="{{ old('jenis_service', $service->jenis_service) }}" required
                                class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-red-500/30 focus:border-red-400 font-bold text-gray-900 transition"
                                placeholder="Contoh: Ganti Selang">
                        ...
                        <div>
                            ...
                            <input type="number" name="biaya" id="biaya" value="{{ old('biaya', $service->biaya) }}" required min="0"
                                class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl focus:ring-2 focus:ring-red-500/30 focus:border-red-400 font-bold text-gray-900 transition">
                ...
                @elseif($service->status_konfirmasi === 'pending')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        {{-- PELANGGAN --}}
                        <div>
