                            ...
                            <div x-data="{ showDone: false }">
                                ...
                                <div x-show="showDone" x-cloak class="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                                    <form action="{{ route('teknisi.tugas.selesai', $task) }}" method="POST" class="space-y-2">
                                        @csrf
                                        <textarea name="catatan" rows="2" class="w-full rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 focus:outline-none focus:border-emerald-400" placeholder="Catatan hasil pekerjaan (opsional)"></textarea>
